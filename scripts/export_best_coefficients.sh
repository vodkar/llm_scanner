#!/usr/bin/env bash
# Materialize the best params of every tuned ranking strategy into config/
# by invoking `llm-scanner export-best-coefficients` once per (study, strategy).
#
# Outputs:
#   config/best_cpg_structural.yaml
#   config/best_current.yaml
#   config/best_evidence_budgeted.yaml
#   config/best_multiplicative_amplification.yaml

set -euo pipefail

cd "$(dirname "$0")/.."

declare -a JOBS=(
  "cpg_structural        cpg_structural          config/best_cpg_structural.yaml"
  "current               current               config/best_current.yaml"
  "evidence_budgeted     evidence_budgeted  config/best_evidence_budgeted.yaml"
  "multiplicative_amplification  multiplicative_amplification  config/best_multiplicative_amplification.yaml"
)

for job in "${JOBS[@]}"; do
  read -r strategy study output <<< "$job"
  echo ">>> ${strategy} <- data/tuning_runs/${study}.db -> ${output}"
  uv run llm-scanner export-best-coefficients \
    --strategy "$strategy" \
    --study-name "$study" \
    --output "$output"
done
